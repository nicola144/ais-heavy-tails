...
def unnormalized_logpdf_Gaussian(x, loc, inv_shape):
    d = len(x)
    assert len(loc) == d and np.shape(inv_shape) == (d, d)

    gap = x - loc
    return -0.5 * np.inner(gap, inv_shape @ gap)

def normalization_Gaussian(d,shape):
    return ((2*np.pi) ** (d/2)) * np.sqrt(np.linalg.det(shape))
